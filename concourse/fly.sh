...
case ${pipeline_config} in
  pr)
      if [ -z "${pipeline_name}" ]; then
          pipeline_name="PR:postgresql-hll"
      fi
      config_file="pr.yml"
      hook_res="postgresql-hll_pr"
    ;;
  commit)
      if [ -z "${pipeline_name}" ]; then
          pipeline_name="COMMIT:postgresql-hll:gpdb"
      fi
      ...
      hook_res="postgresql-hll_commit"
    ;;
  dev)
      if [ -z "${pipeline_name}" ]; then
  ...
  release)
      ...
      if [ -z "${pipeline_name}" ]; then
          pipeline_name="RELEASE:postgresql-hll:${branch}"
      fi
...
yml_path="/tmp/postgresql-hll_pipeline.yml"

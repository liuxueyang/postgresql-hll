...
https://extensions.ci.gpdb.pivotal.io/teams/main/pipelines/PR:postgresql-hll

### Main Branch Pipeline

The development happens on the `gpdb` branch. The commit pipeline for the `gpdb`
https://extensions.ci.gpdb.pivotal.io/teams/main/pipelines/COMMIT:postgresql-hll:gpdb
...
```
./fly.sh -t extension -c dev -p <your_name>_postgresql-hll -b <your_branch>
```
...
The PR pipeline relies on the webhook to detect the new PR. However, due the the limitation of the webhook implemention of concourse, we rely on the push hook for this. It means if the PR is from a forked repo, the PR pipeline won't be triggered immediately. To manually trigger the pipeline, go to https://extensions.ci.gpdb.pivotal.io/teams/main/pipelines/PR:postgresql-hll/resources/postgresql-hll_pr and click ⟳ button there.
